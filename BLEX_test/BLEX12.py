...
def main(argv):
    base_addr = 0x4000000
    p = angr.Project("/home/qinfan/coreutils/coreutils-8.32/src/basename", auto_load_libs=False,
                     load_options={
                         'main_opts': {
                             'base_addr': base_addr
                         }
    ...
    for func in cfg.kb.functions.values():
        ...
        if func.name == 'main':
            ...
            for v in var:
                ...
                if isinstance(v, SimStackVariable):
                    sv.append(v)
                    print(v.name,v.addr)
            print("register variables:",len(rv))
            print("stack variables:", len(sv))
            break
        ...
        def track_mem(state):
            ...
            print("expr:", state.solver.eval(state.inspect.mem_write_expr))
            for gv in gvar:
                if state.inspect.mem_write_address == gv.addr:
