...
def main(argv):
    ...
    file_name = "rm"
    # file_path = "/home/qinfan/coreutils/coreutils-8.32/src/" + file_name
    file_path = "/home/qinfan/coreutils/coreutils-8(1).32/src/" + file_name
    ...
    for func in cfg.kb.functions.values():
        ...
        init_state = p.factory.blank_state(addr=function_start_address, mode="fastpath",
            ...
            for va in var:
                if isinstance(va, SimRegisterVariable):
                    if state.solver.eval(state.inspect.reg_write_offset) == va.reg:
                        # if state.inspect.reg_write_expr in mem_data:
                        #     d = mem_data[state.inspect.reg_write_expr].deref.int.concrete
            ...
            for va in var:
                if isinstance(va, SimStackVariable):
                    ...
                    if state.solver.eval(state.regs.sp) - state.solver.eval(stack_base_addr) == va.addr:
                        # if state.inspect.mem_write_expr in mem_data:
                        #     d = mem_data[state.inspect.mem_write_expr].deref.int.concrete
                        #     var_dict[va.name].append(d)
                        # else:
