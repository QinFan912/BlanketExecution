...
class VarlablesValueKNN:
    def __init__(self, test_path, data_path):
        ...
        self.varNum = 0
        self.valueInVar = defaultdict(int)
    ...
    def load_data(self):
        ...
        for data in datas:
            ...
            for i in set(self.dataValueWeight):
                if i in dataline[1:]:
                    self.valueInVar[i] += 1
    ...
    def weight_setting(self):
        ...
        for i in dataSet:
            ...
            self.weight[i] = round(
                ((total - countDict[i]) / total) * log(self.varNum / self.valueInVar[i]), 4)
        ...
        for i in dataSet:
            countDict[i] = self.dataValueWeight.count(i)
            self.weight[i] = round(
                ((total - countDict[i]) / total) * log(self.varNum / self.valueInVar[i]) / totalWeight, 4)
        countDict = sorted(countDict.items(), key=lambda x: x[1], reverse=True)
        print(countDict)


if __name__ == '__main__':
    test_path = '/home/qinfan/PycharmProjects/angr/data/X86/unexpand_data.txt'
    data_path = '/home/qinfan/PycharmProjects/angr/data/X86/unexpand_data.txt'
